        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                    <h3 class="text-2xl font-bold text-gray-900 dark:text-gray-100 mb-6">Edit Tugas:
                        {{ $assignment->title }}</h3>
                    ...
                    <form method="POST" action="{{ route('admin.assignments.update', $assignment) }}"
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="description" name="description"
                                class="block mt-1 w-full border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-900 text-gray-700 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm"
                                rows="5" required>{{ old('description', $assignment->description) }}</textarea>
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="due_date" class="block mt-1 w-full" type="datetime-local" name="due_date"
                                :value="old(
                                    'due_date',
                                    $assignment->due_date ? $assignment->due_date->format('Y-m-d\TH:i') : '',
                                )" required />
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="max_score" class="block mt-1 w-full" type="number" step="0.01"
                                name="max_score" :value="old('max_score', $assignment->max_score)" min="0" max="100" />
                        ...
                        <div class="mb-4">
                            ...
                            @if ($assignment->file_path)
                                <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">File saat ini: <a
                                        href="{{ Storage::url($assignment->file_path) }}" target="_blank"
                                        class="text-blue-500 hover:underline">{{ basename($assignment->file_path) }}</a>
                                </p>
                                <div class="flex items-center mt-2">
                                    ...
                                    <label for="remove_file" class="ml-2 text-sm text-gray-600 dark:text-gray-400">Hapus
                                        file saat ini</label>
                                </div>
                            @endif
                        </div>
